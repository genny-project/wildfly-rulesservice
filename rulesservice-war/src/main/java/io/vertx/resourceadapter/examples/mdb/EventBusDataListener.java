...
public class EventBusDataListener {
	...
	public CompletionStage<Void> onMessage(Message<String> message) {
		...
		log.info("********* KAFKA DATA LISTENER!!!! *********" + " valid_data came in " + payload.toString());
						...
						if (target != null) {
							...
							if (optea.isPresent()) {
								...
								if (equaled && (!"PRI_SUBMIT".equals(answer.getAttributeCode()))) {
									log.info("This Already exists! " + answer.getAttributeCode() + ":"
											+ answer.getValue());
									changed = false;
								}
							}
		...
		} else if (payload.getString("data_type").equals(GennyItem.class.getSimpleName())) { // Why did I choose this
																								// data_type? ACC
