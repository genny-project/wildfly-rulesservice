...
if [ $XMX ] ; then
  ...
  sed -i 's,-Xms64m,-Xms'"$XMX"',g' /opt/jboss/wildfly/bin/standalone.conf
fi
...
if [[ $DEBUG == "TRUE" ]]; then
   ...
   /opt/jboss/wildfly/bin/standalone.sh --debug -Dorg.kie.task.insecure=true  -Djboss.bind.address.private=127.0.0.1  -bmanagement=0.0.0.0 -b 0.0.0.0 -Dadmin.username=${ADMIN_USERNAME} -Dadmin.password=${ADMIN_PASSWORD} -Dpublic.host=${myip}  -DHIBERNATE_SHOW_SQL=${HIBERNATE_SHOW_SQL:=false} -DHIBERNATE_HBM2DDL=$HIBERNATE_HBM2DDL -DMYSQL_USER=$MYSQL_USER -DMYSQL_PASSWORD=$MYSQL_PASSWORD -Djava.security.auth.login.config=''  -b 0.0.0.0
#--server-config=standalone-full-ha.xml
   ...
   echo "Debug is False";
   /opt/jboss/wildfly/bin/standalone.sh -Dcom.sun.xml.bind.v2.bytecode.ClassTailor.noOptimize=true -Djboss.bind.address.private=127.0.0.1 -Dorg.kie.task.insecure=true  -bmanagement=0.0.0.0 -b 0.0.0.0 -Dadmin.username=${ADMIN_USERNAME} -Dadmin.password=${ADMIN_PASSWORD} -Dpublic.host=${myip}
